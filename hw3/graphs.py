...
def part3():
    ...
    x_train, y_train, x_test, y_test, features_names = load_data('train.csv', 'test.csv')
    for x in [3, 9, 27]:
        classifier = tree.DecisionTreeClassifier("entropy", min_samples_split=x, random_state=2)
        ...
        print(f'x = {x} accuracy = {np.mean(y_pred == y_test)}')
...
def part4():
    print('part 4 -----------------------------------------')
    x_train, y_train, x_test, y_test, features_names = load_data('train.csv', 'test.csv')
    classifier = tree.DecisionTreeClassifier("entropy", min_samples_split=27, random_state=2)
    classifier.fit(x_train, y_train)
...
def part7():
    ...
    classifier = tree.DecisionTreeClassifier("entropy", min_samples_split=27, random_state=0)
    classifier.fit(x_train, y_train)
    y_pred = classifier.predict(x_test)
